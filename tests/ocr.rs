//! Pure-Rust PaddleOCR (PP-OCRv5) engine acceptance test.
//!
//! Loads the offline fixture `tests/fixtures/ocr_sample.png` (720×300, three
//! mixed CJK+Latin lines) into an [`OcrImage`], runs [`PaddleOcr`], and asserts
//! the three reference lines are each recognized (whitespace-stripped substring)
//! with a box in the line's vertical band. A second test recovers a rotated
//! line. Deterministic and offline (the dict is embedded; the ONNX models load
//! from the in-crate `models` dir, or `OCRSPINE_MODELS`).

use ocrspine::{OcrEngine, OcrImage, PaddleOcr};

/// Decodes the fixture PNG into an [`OcrImage`].
fn load_sample() -> OcrImage {
    let bytes = include_bytes!("fixtures/ocr_sample.png");
    OcrImage::from_encoded(bytes).expect("decode sample png")
}
...
fn paddle_recognizes_three_reference_lines() {
    let image = load_sample();
    let engine = PaddleOcr::new().expect("build PaddleOcr");
    let words = engine.recognize(&image).expect("recognize ok");
...
/// Rotated/skewed text. A single CJK+Latin line rendered upright then rotated
/// ~20° (fixture `ocr_rotated.png`). With only axis-aligned detection this line
/// is garbled/missed; minimum-area rotated-rect detection + de-rotation before
/// recognition must recover the bulk of it.
#[test]
fn paddle_recognizes_rotated_line() {
    let bytes = include_bytes!("fixtures/ocr_rotated.png");
    let image = OcrImage::from_encoded(bytes).expect("decode rotated png");
